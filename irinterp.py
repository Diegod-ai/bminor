...
class IRInterpreter:
	'''
	Intérprete para el IR generado por ircode.py.
	...
	def _load_globals(self):
		# Registros temporales para la fase de inicialización global.
		# ircode.py puede emitir MOV + STORE en la sección globals cuando
		# una variable global tiene valor inicial (ej: x: integer = 10).
		self._global_regs: dict[str, Any] = {}
		for inst in getattr(self.program, "globals", []):
			self._exec_global(inst)
		del self._global_regs   # ya no se necesitan
	...
	def _exec_global(self, inst: tuple):
		...
		# MOV: guardar literal en registro temporal global
		if op in {"MOVI", "MOVF", "MOVB", "MOVS"}:
			...
			if op == "MOVF":
				...
				value = int(value)
			self._global_regs[target] = value
			return

		# ADDR: dirección de bloque DATAS
		if op == "ADDR":
			_, name, target = inst
			self._global_regs[target] = name
			return

		# STORE: inicializar variable global con valor calculado
		if op in {"STOREI", "STOREF", "STOREB", "STORES"}:
			_, source, name = inst
			value = self._global_regs.get(source, source)
